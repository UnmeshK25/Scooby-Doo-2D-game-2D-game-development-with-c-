...
SDL_Texture* IOmod::readTexture(const std::string& filename) const{
...
void IOmod::writeText(const std::string& msg, int x, int y,SDL_Color txtcolor) const {
  ...
  SDL_Surface* surface = TTF_RenderText_Solid(font, msg.c_str(), txtcolor);
...
void IOmod:: writeMessage(const std::string& str, double value,int x, int y) const {
  int textWidth;
  int textHeight;
  ...
  SDL_Surface* surface = TTF_RenderText_Solid(font, message.c_str(), txtColor);
  SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, surface);
